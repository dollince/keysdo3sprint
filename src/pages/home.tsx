...
// ─── Модалка ввода сообщения при принятии заявки ────────────────
function AcceptMessageModal({ app, team, onClose, onDone }: {
  ...
  const handleSend = () => {
    ...
    msgs.push({
      ...
      teamName: team.name,
      text: text.trim() || `Ваша заявка в команду "${team.name}" ПРИНЯТА!`,
      read: false,
      createdAt: Date.now(),
    });
    saveMessages(msgs);
      ...
      <div className="modal-card" style={{ maxWidth: '480px' }} onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-body">
          <p style={{ color: '#555', margin: '0 0 12px' }}>
            Applicant: <strong>{app.applicantName}</strong> ({app.applicantEmail})
          </p>
          <div className="form-group-combined">
...
function ApplicationsModal({ team, onClose }: { team: Team; onClose: () => void }) {
  ...
  // Отклонение заявки (автоматическое сообщение)
  const handleReject = (app: Application) => {
    ...
    msgs.push({
      ...
      fromName: localStorage.getItem('userName') || 'Team Owner',
      teamName: team.name,
      text: `К сожалению, ваша заявка в команду "${team.name}" была ОТКЛОНЕНА.`,
      read: false,
      createdAt: Date.now(),
    });
    saveMessages(msgs);
  ...
  // Если открыто окно ввода сообщения
  if (acceptingApp) {
    return (
      <AcceptMessageModal
        app={acceptingApp}
    ...
    <div style={{ maxWidth: '800px', margin: '0 auto', padding: '24px 20px' }}>
      <div className="search-bar-wrapper">
        <div className="search-type-toggle">
          ...
          <button className={`search-type-btn ${searchType === 'people' ? 'active' : ''}`} onClick={() => setSearchType('people')}>People</button>
        </div>
        <input className="search-input" type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder={searchType === 'projects' ? 'Search by name, tag, description...' : 'Search people by name, email or skills...'} />
      </div>
      ...
      {searchType === 'projects' && (
        <>
          ...
          {filteredTeams.length === 0 ? (
            <div className="team-item empty"><p className="empty-text">{trimmedQuery ? 'Nothing found. Try a different query.' : 'No teams yet. Be the first — create one with the + button!'}</p></div>
          ) : (
      ...
      {searchType === 'people' && (
        <>
          ...
          {filteredPeople.length === 0 ? (
            <div className="team-item empty"><p className="empty-text">{trimmedQuery ? 'No people found.' : 'No users registered yet.'}</p></div>
          ) : (
            <div className="teams-list">
              {filteredPeople.map((person, i) => (
                <div className="team-card" key={i} style={{ cursor: 'pointer' }} onClick={() => navigate(`/profile?user=${encodeURIComponent(person.email)}`)}>
                  <div className="team-card-content">
                    <h4 className="team-card-name" style={{ color: '#2d5a3b' }}>{person.name || 'Unknown'}</h4>
                    <p className="team-card-description">{person.email}</p>
                    {person.skills && (
                      <div className="team-tags">
                        {person.skills.split(',').map((s, j) => (<span key={j} className="team-tag">{s.trim()}</span>))}
                      </div>
                    )}
                  </div>
                </div>
