...
import { useNavigate } from 'react-router-dom'; // 👈 Добавлен навигатор
import './profile.css';
import type { Team } from '../App';
import { registerUserProfile, getApplications, saveApplications, getMessages, saveMessages } from './home';
...
const Profile: React.FC<ProfileProps> = ({ teams, onTeamsChange }) => {
  ...
  const [editTeamData, setEditTeamData] = useState<Omit<Team, 'id'>>({ name: '', goals: '', roles: '', description: '' });

  const [viewingTeam, setViewingTeam] = useState<Team | null>(null);
  const [applicationsTeam, setApplicationsTeam] = useState<Team | null>(null);
  const [applications, setApplications] = useState<Application[]>([]);

  // Состояние для модалки ввода сообщения при принятии заявки
  ...
  const closeEditTeam = () => setEditingTeam(null);

  const handleSaveTeam = () => {
    if (!editingTeam) return;
    const updated = teams.map(t => t.id === editingTeam.id ? { ...t, ...editTeamData } : t);
  ...
  const updateApplicationStatus = (appId: number, status: 'accepted' | 'rejected', messageText?: string) => {
    const all: Application[] = JSON.parse(localStorage.getItem('applications') || '[]');
    ...
    if (app && applicationsTeam) {
      msgs.push({
        id: Date.now(),
        toEmail: app.applicantEmail,
        fromName: localStorage.getItem('userName') || 'Team Owner',
        teamName: applicationsTeam.name,
        text: status === 'accepted' 
          ? (messageText || `Ваша заявка в команду "${applicationsTeam.name}" ПРИНЯТА!`) 
          : `К сожалению, ваша заявка в команду "${applicationsTeam.name}" была ОТКЛОНЕНА.`,
        read: false,
        createdAt: Date.now(),
      });
      saveMessages(msgs);
  ...
  const handleAcceptTrigger = (app: Application) => {
    setAcceptingApp(app);
    setAcceptMessage('');
  };
  ...
  const handleAcceptSend = () => {
    ...
    updateApplicationStatus(acceptingApp.id, 'accepted', acceptMessage);
    setAcceptingApp(null);
    setAcceptMessage('');
  };

  const handleRejectTrigger = (app: Application) => {
  ...
  return (
    <div className="profile-container">
      <div className="profile-card">
        ...
        <div className="teams-section">
          ...
          <div className="teams-list">
            {teams.length === 0 ? (
              <div className="team-item empty"><p className="empty-text">No teams yet. Create your first team with the + button!</p></div>
            ) : (
              teams.map(team => (
                <div className="team-card" key={team.id} onClick={() => setViewingTeam(team)} style={{ cursor: 'pointer' }}>
                  ...
                  <button className="team-edit-btn" onClick={(e) => { e.stopPropagation(); openEditTeam(team); }} title="Edit team">✎</button>
                  <button className="applications-btn" onClick={(e) => { e.stopPropagation(); openApplications(team); }} title="View applications">Applications</button>
                </div>
              ))
            )}
          </div>
        ...
        <div className="modal-overlay" onClick={() => setViewingTeam(null)}>
          <div className="modal-card" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
              <div className="form-group-combined"><label>Team Name</label><p className="profile-value">{viewingTeam.name}</p></div>
              <div className="form-group-combined"><label>Project Goals</label><p className="profile-value">{viewingTeam.goals || 'Not specified'}</p></div>
              <div className="form-group-combined"><label>Roles Needed</label><p className="profile-value">{viewingTeam.roles || 'Not specified'}</p></div>
              <div className="form-group-combined"><label>Project Description</label><p className="profile-value">{viewingTeam.description || 'Not specified'}</p></div>
              <div style={{ display: 'flex', gap: '10px', marginTop: '8px' }}>
                <button className="cancel-btn" style={{ flex: 1 }} onClick={() => setViewingTeam(null)}>Close</button>
                <button className="save-btn" style={{ flex: 1 }} onClick={() => {}}>Apply</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {isPasswordModalOpen && (
        <div className="modal-overlay" onClick={closePasswordModal}>
          <div className="modal-card" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
              <div className="form-group-combined"><label>Current Password</label><input type="password" value={currentPassword} onChange={(e) => setCurrentPassword(e.target.value)} placeholder="enter" /></div>
              <div className="form-group-combined"><label>New Password</label><input type="password" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} placeholder="enter" /></div>
              <div className="form-group-combined"><label>Confirm New Password</label><input type="password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder="enter" /></div>
              {passwordError && <p className="password-error">{passwordError}</p>}
              {passwordSuccess && <p className="password-success">{passwordSuccess}</p>}
              <button className="save-btn modal-save-btn" onClick={handlePasswordChange}>Save Password</button>
            </div>
          </div>
        </div>
      )}
      ...
      {editingTeam && (
        <div className="modal-overlay" onClick={closeEditTeam}>
          <div className="modal-card" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
              <div className="form-group-combined"><label>Team Name</label><input type="text" value={editTeamData.name} onChange={(e) => setEditTeamData({ ...editTeamData, name: e.target.value })} placeholder="enter team name" /></div>
              <div className="form-group-combined"><label>Project Goals</label><textarea value={editTeamData.goals} onChange={(e) => setEditTeamData({ ...editTeamData, goals: e.target.value })} placeholder="what do you want to achieve?" rows={3} style={{ background: '#EDF0E8', border: 'none', borderRadius: '0 0 12px 12px', padding: '12px 20px', fontSize: '1rem', color: '#333', outline: 'none', resize: 'vertical', fontFamily: 'inherit' }} /></div>
      ...
      {applicationsTeam && (
        <div className="modal-overlay" onClick={() => { setApplicationsTeam(null); setAcceptingApp(null); }}>
          <div className="modal-card" style={{ maxWidth: '560px' }} onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h2 className="modal-title">Applications — {applicationsTeam.name}</h2>
              <span className="modal-close" onClick={() => { setApplicationsTeam(null); setAcceptingApp(null); }}>X</span>
            </div>
            <div className="modal-body">
              {applications.length === 0 ? (
                <p style={{ color: '#999', fontStyle: 'italic' }}>No applications yet.</p>
              ) : acceptingApp ? (
                // Модалка ввода сообщения при принятии
                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                  <p style={{ color: '#555' }}>Accepting <strong>{acceptingApp.applicantName}</strong></p>
                  <div className="form-group-combined">
                    <label>Message / Contacts</label>
                    <textarea value={acceptMessage} onChange={(e) => setAcceptMessage(e.target.value)} placeholder="Например: Напиши мне в ТГ @username..." rows={3} style={{ background: '#EDF0E8', border: 'none', borderRadius: '0 0 12px 12px', padding: '12px 20px', color: '#333', outline: 'none', resize: 'vertical', fontFamily: 'inherit', fontSize: '1rem' }} />
                  </div>
                  <div style={{ display: 'flex', gap: '10px' }}>
                    <button className="cancel-btn" style={{ flex: 1 }} onClick={() => setAcceptingApp(null)}>Back</button>
                    <button className="save-btn" style={{ flex: 1 }} onClick={handleAcceptSend}>Accept & Send</button>
                  </div>
                </div>
              ) : (
                applications.map(app => (
                  <div key={app.id} className="application-card">
                    <div className="application-info">
                      {/* 👇 Имя стало кликабельным и ведёт на профиль заявителя */}
                      <strong
                        style={{ fontSize: '1rem', color: '#2d5a3b', cursor: 'pointer', textDecoration: 'underline' }}
                        onClick={() => {
                          setApplicationsTeam(null);
                          setAcceptingApp(null);
                          navigate(`/profile?user=${encodeURIComponent(app.applicantEmail)}`);
                        }}
                      >
                    ...
                    <div className="application-status-row">
                      {app.status === 'pending' ? (
                        <>
                          <button className="accept-btn" onClick={() => handleAcceptTrigger(app)}>Accept</button>
                          <button className="reject-btn" onClick={() => handleRejectTrigger(app)}>Reject</button>
                        </>
                      ) : (
